...
function getFushiRoot(app) {
  const smokeRoot = process.env.FUSHI_APPDATA_ROOT?.trim()
  const appDataRoot = smokeRoot || app.getPath('appData')

  return path.join(appDataRoot, 'FUSHI')
}
...
function saveAssetBuffer(app, request) {
  try {
    const asset = request && request.asset

    if (!asset || !Buffer.isBuffer(asset.buffer)) {
      return {
        ok: false,
        error: 'Asset invalido.',
      }
    ...
    ensureDirectory(path.dirname(resolved.targetPath))
    fs.writeFileSync(resolved.targetPath, asset.buffer)
